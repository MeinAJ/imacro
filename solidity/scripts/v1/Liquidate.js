...
const ChainLinkAddress = "0x1e38DCE5381c91F21842a6A80E94e0a06bA4A67A";
...
async function main() {
    ...
    const Chainlink = await ethers.getContractAt("Chainlink", ChainLinkAddress);
    console.log("Chainlink address:", await Chainlink.getAddress());
    ...
    // 再检查一遍清算逻辑
    {
        ...
        // 由deployer2授权Aave2Pool合约可以操作deployer2的usdc
        const approveTx = await USDCToken.connect(deployer2).approve(
            await Aave2Pool.getAddress(),
            2n ** 256n - 1n
        );
        ...
        for (const needLiquidateAddress of addresses) {
            ...
            // 由deployer2清算deployer3的DEGEN借款
            const tx = await Aave2Pool.connect(deployer2).liquidate(
                await DEGENToken.getAddress(),
                borrower,
                ethers.parseUnits("200000", 18), // 大数量，可以全部还完
            )
            await tx.wait();
        }
    }
