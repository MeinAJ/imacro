...
library DynamicInterestRateCalculator {
    ...
    /**
     * @dev 根据风险值计算抵押率调整因子
     * @param _riskFactor 风险值（6位精度，如20% = 200000）
     * @return 抵押率调整因子（6位精度）
     */
    function calculateCollateralizationAdjustment(uint256 _riskFactor) internal pure returns (uint256) {
        // 风险值越高，抵押率调整因子越小
        // 调整因子 = 1 - (风险值 / 2)，最小为50%
        uint256 adjustment = RATE_PRECISION - (_riskFactor / 2);
        return adjustment < 500000 ? 500000 : adjustment; // 最小50%
    }
}
