...
describe("Deposit Lend Withdraw Test", function () {
    ...
    describe("should deposit lend withdraw part of the lend", async () => {
        it("should deposit lend withdraw part of the lend successfully", async () => {
            // 首先授权 Aave2Pool 合约可以操作 address1 的 USDC
            ...
            // 再次存入1000个usdc
            const tx2 = await Aave2Pool.setUserLendLastTimeCalculateFee(address2.address, 1758168833);
            await tx2.wait();
            {
                // 然后调用 depositLend
                ...
                expect(totalBorrow).to.equal(0);
                // 判断借入总金额
                expect(totalLend).to.greaterThan(ethers.parseUnits("2000", 18));
                // 判断address2在平台中的借入总金额
                expect(await Aave2Pool.getUserLend(address2.address)).to.greaterThan(ethers.parseUnits("1000", 18));
                // 判断address2的USDC余额
                ...
                // 判断borrowable
                const degenCollateralDetail = await Aave2Pool.getCollateral(await DEGENToken.getAddress());
                expect(degenCollateralDetail.borrowable).to.greaterThan(ethers.parseUnits("1000", 18));
                const toshiCollateralDetail = await Aave2Pool.getCollateral(await TOSHIToken.getAddress());
                expect(toshiCollateralDetail.borrowable).to.greaterThan(ethers.parseUnits("1000", 18));
            }

            // 只取回2000个usdc
            {
                const tx2 = await Aave2Pool.connect(address2).depositLendWithdraw(ethers.parseUnits("2000", 18));
                await tx2.wait();
                ...
                expect(totalBorrow).to.equal(0);
                // 判断借入总金额
                expect(totalLend).to.greaterThan(ethers.parseUnits("0", 18));
                // 判断address2在平台中的借入总金额
                expect(await Aave2Pool.getUserLend(address2.address)).to.greaterThan(ethers.parseUnits("0", 18));
                // 判断address2的USDC余额
                expect(await USDCToken.balanceOf(address2.address)).to.equal(ethers.parseUnits("100000000", 18));
                // 判断Aave2Pool的USDC余额
                expect(await USDCToken.balanceOf(await Aave2Pool.getAddress())).to.equal(ethers.parseUnits("0", 18));
                // 判断borrowable
                const degenCollateralDetail = await Aave2Pool.getCollateral(await DEGENToken.getAddress());
                expect(degenCollateralDetail.borrowable).to.greaterThan(ethers.parseUnits("0", 18));
                const toshiCollateralDetail = await Aave2Pool.getCollateral(await TOSHIToken.getAddress());
                expect(toshiCollateralDetail.borrowable).to.greaterThan(ethers.parseUnits("0", 18));
            }
        })
    })

    describe("should deposit lend withdraw all of the lend", async () => {
        it("should deposit lend withdraw all of the lend successfully", async () => {
            // 首先授权 Aave2Pool 合约可以操作 address1 的 USDC
            const approveTx = await USDCToken.connect(address2).approve(
                await Aave2Pool.getAddress(),
                2n ** 256n - 1n
            ...
            // 给Aave2Pool mint足够的usdc作为后备资金，防止崩盘
            await USDCToken.mint(await Aave2Pool.getAddress(), ethers.parseUnits("100000000", 18));

            // 取回所有
            {
                const tx2 = await Aave2Pool.connect(address2).depositLendWithdrawAll();
                await tx2.wait();
                expect(tx2.hash).to.not.be.null;
                ...
                expect(totalBorrow).to.equal(0);
                // 判断借入总金额
                expect(totalLend).to.equal(ethers.parseUnits("0", 18));
                // 判断address2在平台中的借入总金额
                expect(await Aave2Pool.getUserLend(address2.address)).to.equal(ethers.parseUnits("0", 18));
                // 判断address2的USDC余额
                expect(await USDCToken.balanceOf(address2.address)).to.greaterThan(ethers.parseUnits("100000000", 18));
                // 判断borrowable
                const degenCollateralDetail = await Aave2Pool.getCollateral(await DEGENToken.getAddress());
                expect(degenCollateralDetail.borrowable).to.equal(ethers.parseUnits("0", 18));
                const toshiCollateralDetail = await Aave2Pool.getCollateral(await TOSHIToken.getAddress());
                expect(toshiCollateralDetail.borrowable).to.equal(ethers.parseUnits("0", 18));
            }
        })
    })
})