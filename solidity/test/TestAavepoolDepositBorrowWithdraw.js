...
describe("deposit borrow withdraw Test", function () {
    ...
    describe("should deposit borrow withdraw successfully", async () => {
        it("should deposit borrow withdraw successfully", async () => {
            ...
            {
                // 首先授权 Aave2Pool 合约可以操作 address3 的 USDC
                const approveTx = await USDCToken.connect(address3).approve(
                    await Aave2Pool.getAddress(),
                    2n ** 256n - 1n
                );
                await approveTx.wait();
                ...
                // withdraw all degen
                const tx = await Aave2Pool.connect(address3).depositBorrowWithdraw(
                    await DEGENToken.getAddress()
                );
                await tx.wait();
                expect(tx.hash).to.not.be.null;
                ...
                expect(totalLend).to.equal(ethers.parseUnits("2000", 18));
                // 判断address3拥有的usdc
                expect(await USDCToken.balanceOf(address3.address)).to.equal(ethers.parseUnits("0", 18));
                // 判断Aave2Pool拥有的DEGEN余额
                expect(await DEGENToken.balanceOf(await Aave2Pool.getAddress())).to.equal(ethers.parseUnits("0", 18));
                // 判断address3的DEGEN余额
                expect(await DEGENToken.balanceOf(address3.address)).to.equal(ethers.parseUnits("100000000", 18));
